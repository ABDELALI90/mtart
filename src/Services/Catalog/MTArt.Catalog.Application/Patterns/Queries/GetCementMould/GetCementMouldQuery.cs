...
namespace MTArt.Catalog.Application.Patterns.Queries.GetCementMould;

public sealed record GetCementMouldQuery(string ReferenceOrSlug, string LanguageCode)
    : IRequest<Result<TilePatternDetailDto>>;

public sealed class GetCementMouldQueryHandler(ICatalogDbContext dbContext)
    : IRequestHandler<GetCementMouldQuery, Result<TilePatternDetailDto>>
{
    public async Task<Result<TilePatternDetailDto>> Handle(GetCementMouldQuery request, CancellationToken cancellationToken)
    {
        ...
        if (pattern is null)
        {
            return Result.Failure<TilePatternDetailDto>(
